...
BANKS = [
    ...
    { "bank": 1, "name": "Preset 1", "size": 64, "type": "Voice", "isROM": True,  },
    { "bank": 2, "name": "Preset 2", "size": 64, "type": "Voice", "isROM": True,  },
]
...
class YamahaSY77(YamahaSYTGBase):
    ...
    def createCustomProgramChange(self, channel: int, patchNo: int) -> List[int]:
        ...
        if   memtype == MemoryType.INTERNAL:  bank_pc = 122
        elif memtype == MemoryType.PRESET1:   bank_pc = 121
        elif memtype == MemoryType.PRESET2:   bank_pc = 117
        else:
    ...
    def friendlyProgramName(self, patchNo: int) -> str:
        ...
        memtype, memnum = self._mapPatchNumToSynthMemory(patchNo)
        memname = ["IN", None, "P1", "P2"][memtype]
        bankchar = "ABCD"[memnum // 16]
    ...
    def bankSlotToPatchNo(self, memory_type: int, memory_number: int) -> int:
        ...
        elif memory_type == self.memory_types["PRESET1"]:   rtn = memory_number + 64
        elif memory_type == self.memory_types["PRESET2"]:   rtn = memory_number + 128
        else:
    ...
    def _mapPatchNumToSynthMemory(self, patchno: int) -> tuple[int, int]:
        ...
        elif patchno <= 127: rtn = (int(self.memory_types["PRESET1"]),  patchno -  64)
        elif patchno <= 191: rtn = (int(self.memory_types["PRESET2"]),  patchno - 128)
        if rtn is None:
...
def make_test_data():
    ...
    def edit_buffers(data: testing.TestData):
        ...
        return [
            testing.ProgramTestData(
                message=edit_buffer, name="Dyna Grand", number=0, rename_name="0123456789")
        ]
    ...
    return testing.TestData(
        ...
        device_detect_reply=(
            [
                ...
                ,
           ], 0
        ),
