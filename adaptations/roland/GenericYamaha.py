...
roland_id = 0x43  # Roland
yamaha_id = 0x43  # Roland
command_rq1 = 0x11
command_dt1 = 0x12
...
class GenericRoland:
    ...
    def getAddressAndSizeFromYamahaMessage(self, message: list) -> Tuple[int, List[int]]:
        size_bits = message[4:6]
        address = message[6:9]
        return size_bits[0] << 7 | size_bits[1], address
    ...
    def isPartOfEditBufferDump(self, message):
        ...
        if self.isOwnSysex(message):
            ...
            size, address = self.getAddressAndSizeFromYamahaMessage(message)
