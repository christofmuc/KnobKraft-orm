...
BANKS = [
    ...
    { "bank": 2, "name": "Internal3", "size": 64, "type": "Voice", "isROM": False, },
    { "bank": 3, "name": "Internal4", "size": 64, "type": "Voice", "isROM": False, },
]
...
class YamahaSY85(YamahaSYTGBase):
    ...
    def createDeviceDetectMessage(self, channel: int) -> List[int]:
        ...
        msg = str2bytes(self.msg_id_voice_dump) + [0x00]*14 + [int(self.memory_types.INTERNAL4), 0]
    ...
    def createCustomProgramChange(self, channel: int, patchNo: int) -> List[int]:
        bankchange  = [0, 3, 6, 9][(patchNo and 0b111000000) >> 6]
        memnum = patchNo and 0b111111
        rtn = [
            0xb0 | (channel & 0x0f), 0x00, bankchange,  # requires SY85 set to Program Change "direct" mode
    ...
    def friendlyBankName(self, bank_number: int) -> str:
        ...
        elif bank_number == 6: rtn = BANKS[2]["name"]
        elif bank_number == 9: rtn = BANKS[3]["name"]
        if rtn is None:
    ...
    def bankSlotToPatchNo(self, memory_type: int, memory_number: int) -> int:
        ...
        elif memory_type == self.memory_types["INTERNAL3"]: rtn = memory_number + 128
        elif memory_type == self.memory_types["INTERNAL4"]: rtn = memory_number + 192
        else:
    ...
    def _validateVoiceMessage(self, buf: List[int]) -> bool:
        rtn = (
            ...
            (   self._getMessageType(buf) == str2bytes(self.msg_id_voice_dump) or
                self._getMessageType(buf) == str2bytes(self.msg_id_drum_dump)
            )
        )
    ...
    def _mapPatchNumToSynthMemory(self, patchno: int) -> tuple[int, int]:
        ...
        elif patchno <= 191: rtn = (int(self.memory_types["INTERNAL3"]), patchno - 128)
        elif patchno <= 255: rtn = (int(self.memory_types["INTERNAL4"]), patchno - 192)
        else:
...
def make_test_data():
    ...
    return testing.TestData(
        ...
        device_detect_call=[
            SYSEX_START, YAMAHA_ID, 0x20, synth.synth_id, 0x4c, 0x4d, 0x20, 0x20, 0x30, 0x30, 0x36, 0x35, 0x56, 0x43, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x09, 0x00, SYSEX_END
        ],
        device_detect_reply=(
            [
