...
class YamahaSYTGBase:
    ...
    def __init__(
        ...
        # memory banks
        memory_types,  # TODO: typing for subclass of IntEnum??
        banks:List[dict],
        ...
        # not user-configurable beyond her
    ...
    def createDeviceDetectMessage(self, channel: int) -> List[int]:
        ...
        bank_name = 'PRESET1' if 'PRESET1' in self.memory_types.__members__ else 'PRESET'
        msg = str2bytes(self.msg_id_voice_dump) + [0x00]*14 + [int(self.memory_types[bank_name]), 0]
    ...
    def createProgramDumpRequest(self, channel: int, patchNo: int) -> List[int]:
        ...
        msg = [*str2bytes(self.msg_id_voice_dump), *[0]*14, int(memory_type), memory_number]
        # print(f"DEBUG ch:{channel} pN:{patchNo}", msg)
        buf = self._makeMessage(device_id, msg, add_checksum=False)
        logging.debug(f"createProgramDumpRequest({channel}, {patchNo}): {bytes2str(buf[:32])}")
        return buf
    ...
    def isSingleProgramDump(self, buf: List[int]) -> bool:
        if self._validateVoiceMessage(buf):
            ...
            logging.debug(f"isSingleProgramDump: msgSize:{msg_size} totalSize:{len(buf)} -> {rtn}")
        else:
            rtn = False
            logging.debug(f"isSingleProgramDump: invalid msg: {buf}")
        return rtn

    def convertToProgramDump(self, channel: int, message: List[int], patchNo: int) -> List[int]:
        buf = message.copy()
        buf[OFFSET_DEVICE_ID] = channel & 0x0f
        buf[self.offset_memory_type], buf[self.offset_memory_number] = self._mapPatchNumToSynthMemory(patchNo)
        buf[OFFSET_CHECKSUM] = self._calculateChecksum(buf)
        logging.debug(f"convertToProgramDump({channel}, msg): "
    ...
    def convertToEditBuffer(self, channel: int, msg: List[int]) -> List[int]:
        ...
        buf[OFFSET_CHECKSUM] = self._calculateChecksum(buf)
        logging.debug(f"convertToEditBuffer({channel}, msg): "
    ...
    def createEditBufferRequest(self, channel: int) -> List[int]:
        device_id = 0x20 | (channel & 0x0f)
        msg = [*str2bytes(self.msg_id_voice_dump), *[0]*14, int(self.memory_types['EDIT']), 0]
        buf = self._makeMessage(device_id, msg, add_checksum=False)
        logging.debug(f"createEditBufferRequest({channel}: {bytes2str(buf[:32])} ... {bytes2str(buf[-2:])}")
        return buf

    def isEditBufferDump(self, buf: List[int]) -> bool:
        rtn = self._validateVoiceMessage(buf) and (buf[self.offset_memory_type] == self.memory_types['EDIT'])
        logging.debug(f"isEditBufferDump: MemType:{buf[self.offset_memory_type]:02x} "
    ...
    def friendlyProgramName(self, patchNo: int) -> str:
        rtn ="OVERRIDE friendlyProgramName()"
        return rtn
    ...
    def _makeMessage(self, device_id: int, msg: List[int], add_checksum: bool=True) -> List[int]:
    ...
    def _validateMessage(self, buf: List[int]) -> bool:
        ...
        rtn = (
            buf[0] == SYSEX_START and
            buf[1] == YAMAHA_ID and
            buf[3] == self.synth_id and
            buf[OFFSET_CHECKSUM] == csum and
            buf[-1] == SYSEX_END
        )
        logging.debug(f"validate_message: {bytes2str(buf[:32])} ... {bytes2str(buf[-2:])}"
    ...
    def __nameFromDump(self, buf: List[int]) -> str:
        return "".join(
            chr(c) for c in buf[self.offset_voice_name:self.offset_voice_name+self.voice_name_length]
        )

    def nameFromDump(self, buf: List[int]) -> str:
        rtn = self.__nameFromDump(buf)
        logging.debug(f"nameFromDump: rtn:{rtn}")
        return rtn

    def numberFromDump(self, buf: List[int]) -> int:
        rtn = self.bankSlotToPatchNo(buf[self.offset_memory_type], buf[self.offset_memory_number])
        logging.debug(f"numberFromDump: MemType/Num: {buf[self.offset_memory_type]}/{buf[self.offset_memory_number]} -> rtn:{rtn} -> rtn:{rtn}")
        return rtn
    ...
    def renamePatch(self, message, new_name) -> List[int]:
        ...
        logging.debug(f"renamePatch({new_name}): {self.__nameFromDump(rtn)}")
        return rtn
    ...
    def blankedOut(self, buf: List[int]) -> List[int]:
        ...
        buf[self.offset_voice_name:self.offset_voice_name+self.voice_name_length] = [0] * self.voice_name_length
        return buf
