...
public:
    ActiveSynthHolder(std::shared_ptr<midikraft::SimpleDiscoverableDevice> synth, Colour const &color)
            : midikraft::SynthHolder(std::move(synth), color) {
    }
...
MainComponent::MainComponent(bool makeYourOwnSize) :
    ...
    if (databaseFile.existsAsFile()) {
        database_ = std::make_unique<midikraft::PatchDatabase>(customDatabase,
                                                               midikraft::PatchDatabase::OpenMode::READ_WRITE);
    } else {
    ...
    LambdaMenuModel::TMenuStructure menuStructure = {
            {0, {"File",       {
                                       {"New database..."},
                                                                       {"Open database..."},
                                                                                                         {"Save database as..."},
                                                                                                                         {"Open recent...", true, 3333, [this]() { return recentFileMenu(); }, [this](
                                                                                                                                 int selected) {
                                                                                                                             recentFileSelected(
                                                                                                                                     selected);
                                                                                                                         }},
    ...
    LambdaButtonStrip::TButtonMap buttons = {
            ...
            {"Edit auto-categories", {"Edit auto-categories", [this]() {
                ...
                EditCategoryDialog::showEditDialog(*database_, this,
                                                   [this](std::vector<midikraft::CategoryDefinition> const &newDefinitions) {
            ...
            {"Edit category import mapping", {"Edit category import mapping", [this]() {
                // This will create the file on demand, copying out the built-in information!
                if (!URL(
                        automaticCategories_->getAutoCategoryMappingFile().getFullPathName()).launchInDefaultBrowser()) {
            ...
            {"Rerun auto categorize...", {"Rerun auto categorize", [this]() {
                ...
                if (AlertWindow::showOkCancelBox(AlertWindow::QuestionIcon, "Re-run auto-categorization?",
                                                 "Do you want to rerun the auto-categorization on the currently filtered " +
                                                 String(affected) + " patches?\n\n"
    ...
    keyboardView_ = std::make_unique<KeyboardMacroView>([this](KeyboardMacroEvent event) {
        switch (event) {
            case KeyboardMacroEvent::Hide:
                patchView_->hideCurrentPatch();
                break;
            case KeyboardMacroEvent::Favorite:
                patchView_->favoriteCurrentPatch();
                break;
            case KeyboardMacroEvent::NextPatch:
                patchView_->selectNextPatch();
                break;
            case KeyboardMacroEvent::PreviousPatch:
                patchView_->selectPreviousPatch();
                break;
            case KeyboardMacroEvent::ImportEditBuffer:
    ...
    addAndMakeVisible(menuBar_);
    //splitter_ = std::make_unique<SplitteredComponent>("LogSplitter", SplitteredEntry{&mainTabs_, 80, 20, 100},
    //                                                  SplitteredEntry{&logArea_, 20, 5, 50}, false);
    ...
    if (persistedSynth.device() && UIModel::instance()->synthList_.isSynthActive(persistedSynth.device())) {
        ...
        if (!UIModel::instance()->synthList_.activeSynths().empty()) {
            auto activeSynth = std::dynamic_pointer_cast<midikraft::Synth>(
                    UIModel::instance()->synthList_.activeSynths()[0]);
            if (activeSynth) {
    ...
    midikraft::MidiController::instance()->setMidiLogFunction(
            [this](const MidiMessage &message, const String &source, bool isOut) {
    ...
    if (makeYourOwnSize) {
        ...
        setSize(static_cast<int>(1024), static_cast<int>(600));
    }
...
void MainComponent::createNewDatabase() {
    ...
    if (databaseChooser.browseForFileToSave(true)) {
        ...
        if (databaseFile.existsAsFile()) {
            if (!AlertWindow::showOkCancelBox(AlertWindow::WarningIcon, "File already exists",
                                              "The file will be overwritten and all contents will be lost! Do you want to proceed?")) {
                return;
            }
        ...
        if (database_->switchDatabaseFile(databaseFile.getFullPathName().toStdString(),
            ...
            Settings::instance().set("LastDatabasePath",
                                     databaseFile.getParentDirectory().getFullPathName().toStdString());
...
void MainComponent::openDatabase() {
    ...
    if (lastPath.empty()) {
        lastPath = File(
                midikraft::PatchDatabase::generateDefaultDatabaseLocation()).getParentDirectory().getFullPathName().toStdString();
    }
...
public:
    explicit MergeAndExport(Array<File> databases) : ThreadWithProgressWindow("Exporting databases...", true, true),
    ...
    void run() override {
        ...
        for (auto const &file: databases_) {
            ...
            if (!exported.exists()) {
                ...
                try {
                    midikraft::PatchDatabase mergeSource(file.getFullPathName().toStdString(),
                                                         midikraft::PatchDatabase::OpenMode::READ_ONLY);
                    auto filter = midikraft::PatchDatabase::allPatchesFilter(allSynths);
                    SimpleLogger::instance()->postMessage(
                            "Exporting database file " + file.getFullPathName() + " containing " +
                            String(mergeSource.getPatchesCount(filter)) + " patches");
                    ...
                    try {
                        ...
                        midikraft::PatchDatabase mergeSource(tempfile.getFullPathName().toStdString(),
                                                             midikraft::PatchDatabase::OpenMode::READ_WRITE_NO_BACKUPS);
                        auto filter = midikraft::PatchDatabase::allPatchesFilter(allSynths);
                        SimpleLogger::instance()->postMessage(
                                "Exporting database file " + file.getFullPathName() + " containing " +
                                String(mergeSource.getPatchesCount(filter)) + " patches");
                        ...
                        SimpleLogger::instance()->postMessage(
                                "Fatal error opening database file " + file.getFullPathName() + ":" + e.what());
                    }
                    tempfile.deleteFile();
                }
                catch (midikraft::PatchDatabaseException &e) {
                ...
                SimpleLogger::instance()->postMessage(
                        "Not exporting because file already exists: " + exported.getFullPathName());
            }
        ...
        SimpleLogger::instance()->postMessage(
                "Done, exported " + String(count) + " databases to pip files for reimport and merge");
    }
...
void MainComponent::exportDatabases() {
    ...
    File lastDirectory(lastPath);
    FileChooser databaseChooser("Please choose a directory with KnobKraft database files that will be exported...",
                                lastDirectory);
...
void MainComponent::mergeDatabases() {
    ...
    File lastDirectory(lastPath);
    FileChooser databaseChooser(
            "Please choose a directory with KnobKraft json files that will be imported and merged...", lastDirectory);
    if (databaseChooser.browseForDirectory()) {
        Settings::instance().set("LastDatabaseMergePath", databaseChooser.getResult().getFullPathName().toStdString());
...
void MainComponent::recentFileSelected(int selected) {
    ...
    if (databaseFile.existsAsFile()) {
        ...
        AlertWindow::showMessageBox(AlertWindow::WarningIcon, "File not found",
                                    "That file no longer exists, cannot open!");
        recentFiles_.removeFile(databaseFile);
        persistRecentFileList();
    }
}
...
void MainComponent::crashTheSoftware() {
    if (AlertWindow::showOkCancelBox(AlertWindow::WarningIcon, "Test software crash",
                                     "This function is to check if the crash information upload works. When you press ok, the software will crash.\n\n"
                                     "Do you feel like it?")) {
...
float MainComponent::calcAcceptableGlobalScaleFactor() {
    ...
    for (auto scale: scales) {
        if (availableHeight * globalScaling_ > 1024 * scale) {
            goodScale = scale;
        }
    }
    return goodScale;
...
void MainComponent::resized() {
    ...
    if (UIModel::instance()->synthList_.activeSynths().size() > 1) {
        auto secondTopRow = area.removeFromTop(LAYOUT_LINE_SPACING + 20 + LAYOUT_INSET_NORMAL)
                .withTrimmedLeft(LAYOUT_INSET_NORMAL).withTrimmedRight(LAYOUT_INSET_NORMAL).withTrimmedTop(
                        LAYOUT_INSET_NORMAL);
...
void MainComponent::refreshSynthList() {
    ...
    if ((!current || (!UIModel::instance()->synthList_.isSynthActive(
            UIModel::instance()->synthList_.synthByName(current->getName()).device())))
        && !listItems.empty()) {
...
void MainComponent::changeListenerCallback(ChangeBroadcaster *source) {
    if (source == midikraft::MidiController::instance()) {
        ...
        auto adaptation = std::dynamic_pointer_cast<knobkraft::GenericAdaptation>(
                UIModel::instance()->currentSynth_.smartSynth());
        Colour tabColour = getUIColour(LookAndFeel_V4::ColourScheme::UIColour::widgetBackground);
        if (adaptation) {
            ...
            if (i != -1) {
                ...
                if (j == -1) {
                    if (UIModel::currentSynth()) {
                        mainTabs_.addTab(UIModel::currentSynth()->getName() + " settings", tabColour,
                                         settingsView_.get(), false, 1);
                    } else {
