...
juce::MidiMessage KawaiK3::requestBankDump(MidiBankNumber bankNo) const
{
...
std::shared_ptr<Patch> KawaiK3::patchFromPatchData(const Synth::PatchData &data, std::string const &name, MidiProgramNumber place) const {
...
{
	return std::vector<juce::MidiMessage>({ patchToSysex(patch, patch.patchNumber()->midiProgramNumber().toZeroBased()) });
}
...
juce::MidiMessage KawaiK3::waveToSysex(const Additive::Harmonics& harmonics)
{
	...
	return patchToSysex(wavePatch, static_cast<int>(KawaiK3::WaveType::USER_WAVE)); //TODO the program number is ignored, that's a shame
}
...
SynthSetup KawaiK3::patchToSynthSetup(PatchData const &patch)
{
	KawaiK3Patch k3patch(MidiProgramNumber::fromZeroBase(0), patch);
	SynthSetup result("unknown patch");

	// Create oscillator information
	auto osc1 = std::make_shared<SimpleOsc>("Osc1", 0, getPasText(KawaiK3Parameter::OSC1_WAVE_SELECT, k3patch), 0);
	osc1->addSpecial(std::make_shared<FlagParameter>(getPasText(KawaiK3Parameter::OSC1_RANGE, k3patch)));
	osc1->addSpecial(std::make_shared<ValueParameter>("Wave", getP(KawaiK3Parameter::OSC1_WAVE_SELECT, k3patch)));
	if (getP(KawaiK3Parameter::LFO_DELAY, k3patch) > 0) {
		// Auto bend is only active when the LFO_DELAY is not 0
		osc1->addSpecial(std::make_shared<ValueParameter>("auto bend", getP(KawaiK3Parameter::OSC_AUTO_BEND, k3patch)));
	}
	auto osc2 = std::make_shared<SimpleOsc>("Osc2", getP(KawaiK3Parameter::OSC2_COARSE, k3patch), getPasText(KawaiK3Parameter::OSC2_WAVE_SELECT, k3patch), 0);
	osc2->addSpecial(std::make_shared<ValueParameter>("Wave", getP(KawaiK3Parameter::OSC2_WAVE_SELECT, k3patch)));
	osc2->addSpecial(std::make_shared<ValueParameter>("detuned", getP(KawaiK3Parameter::OSC2_FINE, k3patch)));
	osc2->addSpecial(std::make_shared<ValueParameter>("balanced", getP(KawaiK3Parameter::OSC_BALANCE, k3patch)));
	result.oscillators_.push_back(osc1);
	result.oscillators_.push_back(osc2);

	// Create the envelope information
	auto vca_envelope = std::make_shared<Envelope>("VCA Envelope", TimeValue::midiValue(0, 0) /* the K3 has no delay in its envelope */,
		TimeValue::midiValue(getP(KawaiK3Parameter::VCA_ATTACK, k3patch), 31), 
		TimeValue::midiValue(getP(KawaiK3Parameter::VCA_DECAY, k3patch), 31),
		ControlValue::midiValue(getP(KawaiK3Parameter::VCA_SUSTAIN, k3patch), 31),
		TimeValue::midiValue(getP(KawaiK3Parameter::VCA_RELEASE, k3patch), 31));
	vca_envelope->setLevel(ControlValue::midiValue(getP(KawaiK3Parameter::VCA_LEVEL, k3patch), 31));
	auto vcf_envelope = std::make_shared<Envelope>("VCF Envelope", TimeValue::midiValue(0, 0) /* the K3 has no delay in its envelope */,
		TimeValue::midiValue(getP(KawaiK3Parameter::VCF_ATTACK, k3patch), 31),
		TimeValue::midiValue(getP(KawaiK3Parameter::VCF_DECAY, k3patch), 31),
		ControlValue::midiValue(getP(KawaiK3Parameter::VCF_SUSTAIN, k3patch), 31),
		TimeValue::midiValue(getP(KawaiK3Parameter::VCF_RELEASE, k3patch), 31));
	vcf_envelope->setLevel(ControlValue::midiValue(getP(KawaiK3Parameter::VCF_ENV, k3patch), 31));

	result.envelopes_.push_back(vcf_envelope);
	result.envelopes_.push_back(vca_envelope);

	return result;
}

std::shared_ptr<Patch> KawaiK3::synthSetupToPatch(SynthSetup const &setup, std::function<void(std::string warning)> logWarning)
{
	auto k3patch = KawaiK3Patch::createInitPatch();

	// Oscillators
	int oscAllocated = 0;
	for (auto osc : setup.oscillators_) {
		auto sosc = std::dynamic_pointer_cast<SimpleOsc>(osc);
		if (sosc) {
			// Allocate this Oscillator, we have 2
			oscAllocated++;
			if (oscAllocated <= 2) {
				int wave = KawaiK3Parameter::findWave(sosc->shape());
				if (wave == -1) {
					wave = KawaiK3Parameter::findWave(Chirp::kSaw());
					logWarning((boost::format("Kawai K3 doesn't have waveform %s, defaulting to Saw") % sosc->shape()).str());
				}
				k3patch->setValue(*KawaiK3Parameter::findParameter(oscAllocated == 1 ? KawaiK3Parameter::OSC1_WAVE_SELECT : KawaiK3Parameter::OSC2_WAVE_SELECT), wave);

				// This is coarse tuning in semitones. The K3 can do this only on Osc2, not Osc1. On Osc1, it has the range switch for 16", 8", and 4"
				if (oscAllocated == 1) {
					int range;
					switch (osc->frequency()) {
					case 0:
						range = 0; // This is 16"
						break;
					case 12:
					...
					case 24:
						...
						break;
					default:
						range = 0; // Default is 16", but we log a warning
						logWarning((boost::format("Ignoring Osc 1 coarse tuning of %d semitones, the Kawai K3 can do only 0, +12, +24 on oscillator 1") % sosc->frequency()).str());
					}
					k3patch->setValue(*KawaiK3Parameter::findParameter(KawaiK3Parameter::OSC1_RANGE), range);
				}
				else {
					// And now, the frequency of the Oscillator 2 is stored absolute, but the K3 needs it relative to Osc1
					int basefrequency = 0;
					switch (k3patch->value(*KawaiK3Parameter::findParameter(KawaiK3Parameter::OSC1_RANGE))) {
					case 0: basefrequency =  0; break;
					case 1: basefrequency = 12; break;
					case 2: basefrequency = 24; break;
					default: jassert(false); 
					}
					k3patch->setValue(*KawaiK3Parameter::findParameter(KawaiK3Parameter::OSC2_COARSE), sosc->frequency() - basefrequency);
				}
			}
			else {
				logWarning((boost::format("Kawai K3 has only 2 oscillators, can't add oscillator #%d") % oscAllocated).str());
			}
		}
		else {
			logWarning((boost::format("Unknown oscillator type for Kawai K3")).str());
		}
	}
	return k3patch;
}
...
juce::MidiMessage KawaiK3::patchToSysex(const Patch &patch, int programNo) const
{
...
void KawaiK3::sendPatchToSynth(MidiController *controller, EditBufferHandler *handler, SimpleLogger *logger, Patch const &patch) {
	MidiBuffer messages;
	messages.addEvent(patchToSysex(patch, kFakeEditBuffer.toZeroBased()), 0);
	auto secondHandler = EditBufferHandler::makeOne();
	handler->setNextEditBufferHandler(secondHandler, [this, handler, logger, controller, secondHandler](MidiMessage const &message) {
		if (isWriteConfirmation(message)) {
			handler->removeEditBufferHandler(secondHandler);
