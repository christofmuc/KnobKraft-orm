...
class KawaiK3: public Synth, public SupportedByBCR2000, public SynthPatchCapability, public ProgramDumpCabability, public BankDumpCapability, 
...
public:
	...
	std::shared_ptr<Patch> patchFromPatchData(const Synth::PatchData &data, std::string const &name, MidiProgramNumber place) const override;

	// Special Synth implementation
	virtual void sendPatchToSynth(MidiController *controller, EditBufferHandler *handler, SimpleLogger *logger, Patch const &patch) override;
	...
	// SynthPatchCapability
	virtual SynthSetup patchToSynthSetup(PatchData const &patch) override;
	...
	MidiMessage patchToSysex(const Patch &patch, int programNo) const;

	void retrieveWave(MidiController *controller, EditBufferHandler *handler, SimpleLogger *logger, SynthView *synthView);
	void sendK3Wave(MidiController *controller, EditBufferHandler *handler, SimpleLogger *logger, Patch *patch, MidiMessage const &userWave);
...
private:
	...
	EditBufferHandler::HandlerHandle k3BCRSyncHandler_ = EditBufferHandler::makeNone();
