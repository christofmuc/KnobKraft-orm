...
juce::MidiMessage Matrix1000::requestBankDump(MidiBankNumber bankNo) const
{
	...
	return createRequest(BANK_AND_MASTER, bankNo.toZeroBased());
}
...
std::vector<juce::MidiMessage> Matrix1000::requestPatch(int programNumber)
{
	...
	result.push_back(createRequest(SINGLE_PATCH, programNumber));
	return result;
}
...
std::shared_ptr<Patch> Matrix1000::patchFromPatchData(const Synth::PatchData &data, std::string const &name, MidiProgramNumber place) const {
...
Synth::PatchData Matrix1000::filterVoiceRelevantData(PatchData const &unfilteredData) const
{
	...
	return Patch::blankOut(kMatrix1000BlankOutZones, unfilteredData);
}
...
void Matrix1000::changeInputChannel(MidiController *controller, MidiChannel channel)
{
