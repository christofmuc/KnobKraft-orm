...
MKS80_Patch::MKS80_Patch(std::shared_ptr<MKS80_PatchNumber> patchNumber, std::map<APR_Section, std::vector<uint8>> const &data) : patchNumber_(patchNumber)
{
...
MKS80_Patch::MKS80_Patch(MidiProgramNumber patchNumber, Synth::PatchData const &data) : Patch(data)
{
...
std::vector<std::string> MKS80_Patch::warnings()
{
	throw std::logic_error("The method or operation is not implemented.");
}

std::vector<SynthParameterDefinition *> MKS80_Patch::allParameterDefinitions()
{
