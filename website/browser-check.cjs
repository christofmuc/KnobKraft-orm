...
(async () => {
  ...
  assert.match(await page.locator('#synthTableBody').innerText(), /Nord Lead/);
  assert.doesNotMatch(await page.locator('#synthTableBody').innerText(), /After 2.9.0/);
  ...
  await page.goto(new URL('docs/learn/overview/', base).href, { waitUntil: 'networkidle' });
  assert.equal(await page.locator('iframe').count(), 0, 'Tutorial must not embed YouTube before consent');
  assert(await page.locator('.video-preview img').isVisible(), 'Local video preview is missing');
  assert.match(await page.locator('.video-preview').getAttribute('href'), /^https:\/\/www\.youtube\.com\/watch\?/);
