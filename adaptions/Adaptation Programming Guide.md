...
This method should return a MIDI message in the form of a list of byte-values integers used to detect the device. Most often, this is the Identity Request message from the list of Universal Sysex messages (https://www.midi.org/specifications-old/item/table-4-universal-system-exclusive-messages), but it could be anything, really.
...
Similar to the `createDeviceDetectMessage`, this function should return a MIDI message that makes the device send its Edit Buffer. In case the device doesn't support this operation, you can send it a command to return a specific program number maybe.
...
Parameters givem are `channel`, the channel detected, and the `message`, which is the patch stored in the database. 
...
This code contains a security check in that the message handed in is actually an edit buffer dump - program errors might cause something else to end up here, and throws an exception in case this is the case. Don't worry, a regular Exception from Python will not crash the Orm, but rather create a print message in the log window.
